...
export const getTradeInput = async (sourceToken: string, targetToken: string, targetAmount: number) => {
    try {
        ...
        if (isValidSourceToken && isValidTargetToken) {
            ...
            const data = await contract.tradeInputByTargetAmount(sourceTokenToUse, targetTokenToUse, parseUnits(String(targetAmount), "ether"));
