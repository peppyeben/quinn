...
export const getTradeOutput = async (sourceToken: string, targetToken: string, sourceAmount: number) => {
    try {
        ...
        if (isValidSourceToken && isValidTargetToken) {
            ...
            const sourceTokenToUse = isAddress(sourceToken) ? sourceToken : BANCOR_POOL_TOKENS[sourceTokenKey];
            const targetTokenToUse = isAddress(targetToken) ? targetToken : BANCOR_POOL_TOKENS[targetTokenKey];

            const contract = await tradeContract();

            const data = await contract.tradeOutputBySourceAmount(sourceTokenToUse, targetTokenToUse, parseUnits(String(sourceAmount), "ether"));
